...
class Dailies(commands.Cog):
    ...
    async def energy_listener(self, discord_id, name, offset, energy_timing):
        embed = discord.Embed(description=f"Free energy for **{name}** is available")
        ...
        try:
            ...
            reminder_embed = discord.Embed(description=f"30 minutes left to claim free energy")
    ...
    async def dailies_listener(self, discord_id, name, offset):
        ...
        try:
            ...
            reminder_embed = discord.Embed(description=f"1 hour left until reset\nReminder for {name} to do dailies")
